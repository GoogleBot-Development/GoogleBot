...
.addField(":bust_in_silhouette: Username", user.tag)
.addField(":id: User ID", user.id)
.addField(":label: Status", status[member.presence.status])
.addField(":video_game: Playing", `${presence.name}` || "Not playing a game")
.addField(":calendar_spiral: Account Created", require('dateformat')(user.createdAt, "mmmm d, yyyy"))
.addField(`:calendar: Joined ${message.guild.name} on`, require('dateformat')(member.joinedAt, "mmmm d, yyyy"))
