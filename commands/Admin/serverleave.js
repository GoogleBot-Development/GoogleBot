...
if(message.author.id !== "667354950321569792") return
  ...
  let server = message.client.shard.broadcastEval("message.client.guilds.cache.get(args[0])")
  let reason = args.slice(1).join(" ")
  if(!reason) return message.channel.send("You must give me a reason to make me leave the server!")
  message.client.shard.broadcastEval("message.client.guilds.cache.get(server.id).leave()")
