...
if(message.author.id !== "667354950321569792") return
 if(!args[0]) return message.channel.send("You must give a server ID to make me leave!")
  ...
  message.client.shard.broadcastEval("this.guilds.cache.get(args[0]).leave()")
  
  message.channel.send(`I have just left: ${server.name} (${args[0]}) with reason: ${reason}`)
  
  console.log(`[Shard #${fish}] I was forced to leave: ${server.name} (${args[0]}) with reason: ${reason}`)
}
